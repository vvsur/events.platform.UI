...
function formatDates(momentStartUnix, momentEndUnix) {
  ...
  if (momentStart.format("DDMMYYYY") === momentEnd.format("DDMMYYYY")) {
    ...
    momentText =
      formatMonth(momentStart.format("dd, DD MMM YYYY")) +
      " - " +
      formatMonth(momentEnd.format("dd, DD MMM YYYY"));
  }
...
const EventsListItem = (props) => {
  ...
  if (size === "small")
    return (
      <div className="flex border-gray-400 p-2">
        {item.image_url && (
              ...
              <Link href={itemUrlHref} as={itemUrlAs}>
                <a>
                  <img
                    ...
                    src={item.image_url}
                    alt={jsUcfirst(item.title) + " на GonnaVisit.com"}
                    title={jsUcfirst(item.title) + " на GonnaVisit.com"}
                  />
                </a>
              </Link>
            }
        ...
        <div className="mt-0 ml-6">
          <div className="block mb-2">
            <Link href={itemUrlHref} as={itemUrlAs}>
              <a className="text-base font-semibold text-gray-900 no-underline border-b-2 border-white hover:border-teal-500 hover:text-gray-900">
                {jsUcfirst(item.title)}
              </a>
            </Link>
          </div>
          <div>
            ...
            {item.is_free === true ? (
              <span className="text-sm text-indigo-600">Бесплатно</span>
            ) : (
    ...
    return (
      <div
        className={
          props.index === 0
            ? "md:flex border-gray-400 p-4 py-6"
            : "md:flex border-t border-gray-400 p-4 py-6"
        }
      >
        {item.image_url && (
        ...
        <div className="mt-4 md:mt-0 md:ml-6">
          <div className="block mb-2">
            <Link href={itemUrlHref} as={itemUrlAs}>
              <a className="text-lg font-semibold text-gray-900 no-underline border-b-2 border-white hover:border-teal-500 hover:text-gray-900">
                {jsUcfirst(item.title)}
              </a>
            </Link>
          </div>
