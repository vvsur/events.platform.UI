...
function Page({ data, current }) {
  ...
  function getPageTitle(citySysName, momentSysName) {
  ...
  let pageTitle = getPageTitle(city, moment, group);
...
Page.getInitialProps = async ({ query }) => {
  ...
  const eventsRes = await fetch(`${process.env.API_URL}/events?page=${(!page) ? 0 : page}&page_size=${pageSize}&city=${city}&moment=${moment}&group=${group}`)
